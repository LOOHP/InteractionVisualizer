...
public class V1_14 extends NMS {
	...
	public String getBannerCustomName(Block block) {
		return ((CraftWorld) block.getWorld()).getHandle().getTileEntity(new net.minecraft.server.v1_14_R1.BlockPosition(block.getX(), block.getY(), block.getZ())).b().getString("CustomName");
	}
	
	@SuppressWarnings("resource")
	@Override
	public WrappedCollection<?, Entity> getEntities(World world) {
		return new WrappedCollection<net.minecraft.server.v1_14_R1.Entity, Entity>(((CraftWorld) world).getHandle().entitiesById.values(), entry -> entry.getBukkitEntity());
	}

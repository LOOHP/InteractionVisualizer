...
public class V1_13_1 extends NMS {
	...
	public String getBannerCustomName(Block block) {
		return ((CraftWorld) block.getWorld()).getHandle().getTileEntity(new net.minecraft.server.v1_13_R2.BlockPosition(block.getX(), block.getY(), block.getZ())).aa_().getString("CustomName");
	}
	
	@SuppressWarnings("resource")
	@Override
	public WrappedCollection<?, Entity> getEntities(World world) {
		return new WrappedCollection<net.minecraft.server.v1_13_R2.Entity, Entity>(((CraftWorld) world).getHandle().entityList, entry -> entry.getBukkitEntity());
	}

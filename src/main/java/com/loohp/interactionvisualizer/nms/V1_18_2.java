...
public class V1_18_2 extends NMS {
    ...
    public String getBannerCustomName(Block block) {
        return ((CraftWorld) block.getWorld()).getHandle().c_(new net.minecraft.core.BlockPosition(block.getX(), block.getY(), block.getZ())).aa_().l("CustomName");
    }

    @SuppressWarnings("resource")
    @Override
    public WrappedIterable<?, Entity> getEntities(World world) {
        return new WrappedIterable<net.minecraft.world.entity.Entity, Entity>(((CraftWorld) world).getHandle().O.d().a(), entry -> entry.getBukkitEntity());
    }

...
public class V1_11 extends NMS {
	...
	public int getItemDespawnRate(Item item) {
		int despawnRate;
		try {
			Object spigotWorldConfig = net.minecraft.server.v1_11_R1.World.class.getField("spigotConfig").get(((CraftWorld) item.getWorld()).getHandle());
	...
	public String getBannerCustomName(Block block) {
		return ((CraftWorld) block.getWorld()).getHandle().getTileEntity(new net.minecraft.server.v1_11_R1.BlockPosition(block.getX(), block.getY(), block.getZ())).d().getString("CustomName");
	}
	
	@Override
	public WrappedCollection<?, Entity> getEntities(World world) {
		return new WrappedCollection<net.minecraft.server.v1_11_R1.Entity, Entity>(((CraftWorld) world).getHandle().entityList, entry -> entry.getBukkitEntity());
	}
